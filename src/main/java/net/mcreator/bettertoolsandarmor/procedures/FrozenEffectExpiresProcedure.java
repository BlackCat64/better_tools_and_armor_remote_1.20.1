...
public class FrozenEffectExpiresProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		}
		{
			...
			if (!_ent.level().isClientSide() && _ent.getServer() != null) {
				_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
						_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), "kill @e[type=minecraft:block_display,nbt={ForgeData:{freeze_effect:1b}},sort=nearest,limit=1]");
			}
		}
		BetterToolsMod.queueServerWork(5, () -> {
			entity.setTicksFrozen(55);
		});
		entity.getPersistentData().putBoolean("is_frozen", false);
	}
}
